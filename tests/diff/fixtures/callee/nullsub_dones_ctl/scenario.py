# Scenario "nullsub_dones_ctl": the control for "nullsub_dones". Synthetic data only. Regenerate with
#   python -B tests/diff/fixtures/callee/gen_callee_fixtures.py --diaphora-dir <diaphora-ref> --only nullsub_dones_ctl --no-walk
#
# The same databases, except that hub_fn's changed pseudo-code line calls other_fn instead of nullsub_7, so
# no nullsub candidate key enters dones. The nullsub_7 -> nullsub_7 seed is then walked and its pseudo-code
# diff finds leaf_old_fn -> leaf_new_fn: the pair nullsub_dones leaves unmatched passes every gate, so its
# absence there is due to the dones-before-nullsub order alone.
...
MAIN = {
    "functions": [
        Mk(1, "hub_fn", 0x1000, 1, HUB_ASM, Pseudo("other_fn", "v1"), Nodes=12, Bh="hub"),
...
DIFF = {
    "functions": [
        Mk(1, "hub_fn", 0x1000, 1, HUB_ASM, Pseudo("other_fn", "v2"), Nodes=12, Bh="hub"),
