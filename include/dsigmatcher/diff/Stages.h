...
// One function per Diaphora pass. All of them run on the single diff thread, in Diaphora's order,
// called by RunPipeline / RunReplay (Pipeline.h).
//
// Points and cleanups. RunPipeline emits the before/after points of the top-level stages it calls
// (the point names are listed in tools/parity/README.md). A stage function itself must:
...
//   * emit, with S.Point(...), the points of the Diaphora methods it calls that the oracle
...
// Errors: throw DiaphoraWouldRaise at the Python raise site (01 §13, 07 §10.15). Heuristic workers
// truncate that heuristic and continue (inside StageRunHeuristicsForCategory); main-thread passes let
// it propagate. UnsupportedInput (a quirk the port refuses) and IoFailure / SqliteEnvironmentFailure
// (the disk, the temporary directory, a damaged file) are never caught by a stage: they end the run
// (Errors.h).
...
// ---- stages/RelatedCompilationUnit.cpp ------------------------------------------------------
