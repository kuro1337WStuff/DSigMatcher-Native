...
- **Runtime used for experiments.** `<python-home>/python.exe` = Python 3.13.12, `sqlite3.sqlite_version` = 3.51.1, `cdifflib` not installed. `unified_diff` always comes from stdlib `difflib`, even when `cdifflib` is installed (see `diaphora.py:51`).
