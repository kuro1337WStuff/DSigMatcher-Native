...
- **The reference was not modified.** All experiments ran against a copy of Diaphora at `<scratch>/dcopy` (made with `tar --exclude=.git --exclude=__pycache__`) on copies of the databases. `<scratch>` is a scratch directory. Everything there is ephemeral and not committed.
- **Real test data.** While this spec was being written, real IDA/Diaphora exports were produced under `<corpus>/oracle/exports/`. They were copied to `<scratch>/../real/` before use and never opened in place:
...
- **Checks performed.**
  ...
  7. The plain harness is the CLI. Its output for both of those pairs is identical, including `line`, to the oracle's `python diaphora.py` runs (`oracle/diffs/ls-old_vs_ls/run1/ls-old_vs_ls.diaphora` and `oracle/diffs/userenv-9168-pdb_vs_9278-pdb/run1/…`).
